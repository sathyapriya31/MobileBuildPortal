...
export default function BuildPage() {
  ...
  const wsState = location.state?.fromWorkspace ? location.state : null;
  const [pendingWorkspaceBranch, setPendingWorkspaceBranch] = useState(
    wsState?.workspaceConfig?.branch || null
  );
  const workspaceApplied = useRef(false);
  ...
  useEffect(() => {
    if (wsState && user && !workspaceApplied.current) {
      workspaceApplied.current = true;
      dispatch(selectRepo({
        id: wsState.workspaceRepo.id,
        name: wsState.workspaceRepo.name,
        fullName: wsState.workspaceRepo.fullName,
        cloneUrl: wsState.workspaceRepo.cloneUrl,
      }));
      const cfg = wsState.workspaceConfig;
  ...
  useEffect(() => {
    if (branches.length) {
      if (pendingWorkspaceBranch) {
        const match = branches.find(b => b.name === pendingWorkspaceBranch);
        setBranch(match ? match.name : branches[0].name);
        setPendingWorkspaceBranch(null);
      } else {
