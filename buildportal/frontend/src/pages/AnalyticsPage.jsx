...
import { ChevronDown, ChevronRight, Calendar, CheckCircle2, AlertCircle, XCircle, AlertTriangle, Clock, Cpu, Code2 } from 'lucide-react';
...
function KpiCard({ title, value, trend, trendColor, iconBg, icon, styles }) {
  return (
    <div style={styles.kpiCard} className="analytics-metric-card">
...
function PlatformDonutChart({ android, ios, total, styles }) {
  ...
  const hasGap = total > 0 && android > 0 && ios > 0;

  // Compute startAngle so iOS segment is always centered at 9 o'clock (left side).
  // In Recharts: 0°=right, 90°=top, clockwise = decreasing angle.
  // iOS center target = 180° (left). iOS occupies iosDegrees total, so it starts
  // at 180° + iosDegrees/2 (clockwise). Android (first segment) ends there, so:
  //   startAngle = 180 + iosDegrees/2 + androidDegrees
  const iosDegrees = total > 0 ? (ios / total) * 360 : 0;
  const androidDegrees = total > 0 ? (android / total) * 360 : 0;
  const rawStart = total > 0 ? 180 + iosDegrees / 2 + androidDegrees : 90;
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '20px' }}>
        ...
        <div style={{ position: 'relative', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ position: 'absolute', textAlign: 'center', pointerEvents: 'none' }}>
            <div style={{ fontSize: '23px', fontWeight: '600', color: '#111827', fontFamily: 'Google Sans, sans-serif', lineHeight: 1 }}>
              {total.toLocaleString()}
            </div>
            <div style={{ fontSize: '11px', color: '#9CA3AF', fontFamily: 'Google Sans, sans-serif', marginTop: '5px', fontWeight: '400' }}>
              Total Builds
            </div>
          </div>
        </div>
